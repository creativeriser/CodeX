...
import { ShieldCheck, GraduationCap, LineChart, Globe2, Brain, Zap, CheckCircle2, AlertTriangle, ArrowRight, ScanFace, User } from "lucide-react"
                            ...
                            >
                                ...
                                <div className="p-8 relative bg-white">
                                    <div className="text-center mb-8">
                                        <h3 className="text-xl font-bold text-slate-900 tracking-tight">Vishu Singh</h3>
                                        <p className="text-xs text-slate-500 font-medium mt-1">Computer Science • Class of 2023</p>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {["React Framework", "Node.js Env", "System Design", "Academic (7.9 CGPA)", "Attendance (65%)"].map((skill, i) => (
                                            <motion.div
                                                key={skill}
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-1.5 h-1.5 rounded-full ${i === 2 ? "bg-slate-300" : (i > 2 ? "bg-purple-500" : "bg-emerald-500")}`}></div>
                        ...
                        >
                            ...
                            >
                                <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
                                    ...
                                    <div>
                                        <div className="text-sm font-bold text-slate-900 leading-none">Vishu Singh</div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="flex justify-between items-center p-2.5 rounded-lg bg-slate-50 border border-slate-100">
                                        ...
                                        <span className="text-xs font-bold text-blue-600">7.9</span>
                                    </div>
                                    <motion.div
...
function GapMotionCore({ active }: { active: boolean }) {
    // Conflict Engine: Misaligned Rings + Warning Pulse
    return (
        <div className="w-[140px] h-[140px] flex items-center justify-center relative pointer-events-none">
            <svg width="140" height="140" viewBox="-70 -70 140 140" className="overflow-visible">
                {/* 1. Unstable Orbit */}
                <motion.g animate={{ rotate: 360 }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }}>
                    <circle r="60" stroke="#f43f5e" strokeWidth="0.5" strokeDasharray="4 2" fill="none" opacity="0.4" />
                </motion.g>

                {/* 2. Misaligned Segments */}
                <motion.g
                    animate={active ? { rotate: [0, 45, 0], scale: [1, 1.1, 1] } : {}}
                    transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                >
                    <path d="M-40,0 A40,40 0 0,1 40,0" stroke="#f43f5e" strokeWidth="2" fill="none" opacity="0.8" />
                    <path d="M-40,10 A40,40 0 0,0 40,10" stroke="#f43f5e" strokeWidth="2" fill="none" opacity="0.5" />
                </motion.g>

                {/* 3. Pulsing Warning Core */}
                <motion.circle
                    r="15" fill="#f43f5e" opacity="0.2"
            ...
            <div className="absolute -bottom-6 text-[8px] font-bold text-rose-500 uppercase tracking-[0.2em] animate-pulse">Conflict Detected</div>
        </div>
    )
}

function BridgeMotionCore({ active }: { active: boolean }) {
    // Synthesis Engine: Interlocking Gears/Segments
    return (
        <div className="w-[140px] h-[140px] flex items-center justify-center relative pointer-events-none">
            <svg width="140" height="140" viewBox="-70 -70 140 140" className="overflow-visible">
                {/* 1. Base Ring */}
                <circle r="55" stroke="#f59e0b" strokeWidth="1" strokeDasharray="2 4" fill="none" opacity="0.3" />

                {/* 2. Filling Segments */}
                <motion.path
                    d="M0,-50 A50,50 0 0,1 50,0" stroke="#f59e0b" strokeWidth="3" fill="none" strokeLinecap="round"
                ...
                <motion.rect
                    width="20" height="20" x="-10" y="-10" rx="4"
                    fill="none" stroke="#f59e0b" strokeWidth="2"
                    animate={{ rotate: 90, scale: [1, 0.8, 1] }}
                    transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                />
                <motion.rect
                    width="12" height="12" x="-6" y="-6" rx="2"
                    fill="#f59e0b"
                    animate={{ rotate: -90, opacity: [0.5, 1, 0.5] }}
                    transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                />
            </svg>
            <div className="absolute -bottom-6 text-[8px] font-bold text-amber-500 uppercase tracking-[0.2em]">Synthesis Active</div>
        </div>
    )
}

function MatchMotionCore({ active }: { active: boolean }) {
    // Alignment Engine: Perfect Locking Rings
    return (
        <div className="w-[140px] h-[140px] flex items-center justify-center relative pointer-events-none">
            <svg width="140" height="140" viewBox="-70 -70 140 140" className="overflow-visible">


                {/* 2. Central Verified Check - Scaled Up */}
                <motion.g animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}>
                    <motion.circle
                        r="28" fill="#10b981" fillOpacity="0.2" stroke="#10b981" strokeWidth="2"
                        initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.5, type: "spring" }}
                    />
                    <motion.path
                        d="M-12,2 L-4,10 L12,-10" stroke="#10b981" strokeWidth="4" fill="none" strokeLinecap="round" strokeLinejoin="round"
                        initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ delay: 0.8, duration: 0.5 }}
                    />
                </motion.g>
            </svg>
            <div className="absolute -bottom-6 text-[8px] font-bold text-emerald-500 uppercase tracking-[0.2em]">Match Verified</div>
        </div>
    )
}
